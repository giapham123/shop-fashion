...
const getProductsMenu = (type) =>
  db.products.filter((item) => item.for.cate === type);
...
Mock.onPost("/api/fashion-shop-2/get-products-fashion").reply(async (config) => {
  try {
    // const requestData = JSON.parse(config);
    console.log('Received receipt data:', config.data);
